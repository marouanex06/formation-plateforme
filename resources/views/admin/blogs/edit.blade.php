    ...
    <div class="bg-white rounded-xl shadow p-8">
        <form method="POST" action="{{ route('admin.blogs.update', $blog) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title_fr" value="{{ old('title_fr', $blog->title_fr) }}"
                    class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>

            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700 mb-1">Titre EN</label>
                <input type="text" name="title_en" value="{{ old('title_en', $blog->title_en) }}"
                    class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="status"
                    ...
                    <option value="published" {{ old('status', $blog->status) == 'published' ? 'selected' : '' }}>Publie</option>
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea name="content_fr" rows="6"
                    class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('content_fr', $blog->content_fr) }}</textarea>
            </div>

            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-1">Contenu EN</label>
                <textarea name="content_en" rows="6"
                    class="w-full border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('content_en', $blog->content_en) }}</textarea>
            </div>
